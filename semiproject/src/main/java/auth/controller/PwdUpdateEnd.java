...
public class PwdUpdateEnd extends AbstractController {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // 기본 검증
        if (userid.isEmpty() || email == null || email.trim().isEmpty()) {
            message = "비정상적인 접근입니다. 다시 시도해주세요.";
        ...
        // 임시비번 생성(영문+숫자 10자리)
        String tempPwd = makeTempPassword(10);
        ...
        if (n == 1) {
            try {
                ...
                message = "임시비밀번호가 이메일로 발급되었습니다. 로그인 후 비밀번호를 변경하세요.";
            } catch (Exception e) {
                e.printStackTrace();
                message = "임시비밀번호 발급은 되었으나, 메일 전송에 실패했습니다. 관리자에게 문의하세요.";
            }
        } else {
    ...
    private void goMsg(HttpServletRequest request, String message, String loc) throws Exception {
    ...
    // 패스워드 만들기 
    private String makeTempPassword(int len) {
