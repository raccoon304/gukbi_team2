...
public class PaymentFail extends AbstractController {
        ...
        try {
            ...
            if (pgTid != null && !pgTid.isBlank()) {
                ...
                if (cancelSuccess) {
                    ...
                    System.out.println("⚠️ PG 결제 취소 실패 - 수동 환불 필요!");
            ...
            if (statusUpdateResult > 0) {
                ...
                for (Map<String, Object> detail : orderDetails) {
                    ...
                    if (optionId == 0 || quantity == 0) {
                        System.out.println("⚠️ 잘못된 데이터: optionId=" + optionId + ", quantity=" + quantity);
                        totalFailed++;
                        continue;
                    }
                    ...
                    if (result > 0) {
                        ...
                        totalFailed++;
                        System.out.println("⚠️ 재고 복구 실패: optionId=" + optionId);
                    }
                }
                ...
                if (totalFailed > 0) {
                    System.out.println("⚠️ 재고 복구 실패 항목 있음 - 수동 확인 필요!");
                }

                System.out.println("✅ 결제 실패 처리 완료 (orderId: " + orderId + ")");
                
            } else {
                System.out.println("⚠️ 이미 처리된 주문 또는 READY 상태 아님 (orderId: " + orderId + ")");
            }
    ...
    private boolean cancelPGPayment(String pgTid, int orderId) {
        try {
            ...
            System.out.println("⚠️ PG 결제 취소 API 미구현 상태");
            System.out.println("⚠️ 관리자 페이지에서 수동 환불 처리 필요:");
            System.out.println("   - 주문번호: " + orderId);
